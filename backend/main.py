...
class ProductionModelManager:
    ...
    def _build_unet_efficientnet_architecture(self) -> Model:
        """Architecture UNet EfficientNet - EXACTE du notebook d'entraînement"""
        # Backbone EfficientNetB0 (même que l'entraînement)
        ...
        # Extraire feature maps à différents niveaux
        inputs = backbone.input
        
        # Skip connections EfficientNet
        skip1 = backbone.get_layer('block2a_expand_activation').output  # 256x512
        skip2 = backbone.get_layer('block3a_expand_activation').output  # 128x256
        skip3 = backbone.get_layer('block4a_expand_activation').output  # 64x128
        skip4 = backbone.get_layer('block6a_expand_activation').output  # 32x64
        
        # Bottleneck
        bottleneck = backbone.output  # 16x32
        ...
        # Output (8 classes Cityscapes)
        outputs = layers.Conv2D(NUM_CLASSES, 1, activation='softmax', name='output_segmentation')(x)
        
        return Model(inputs=[inputs], outputs=[outputs], name='unet_efficientnet_fallback')
    
    def _build_deeplabv3plus_architecture(self) -> Model:
        """Architecture DeepLabV3+ - EXACTE du notebook d'entraînement"""
        ...
        gap = layers.Conv2D(256, 1, activation='relu')(gap)
        gap = layers.UpSampling2D((x.shape[1], x.shape[2]), interpolation='bilinear')(gap)
        
        # Concatenate ASPP features
        aspp_concat = layers.concatenate([aspp1, aspp2, aspp3, aspp4, gap])
        aspp_output = layers.Conv2D(256, 1, padding='same', activation='relu')(aspp_concat)
        ...
        x = layers.UpSampling2D((4, 4), interpolation='bilinear')(aspp_output)
        ...
        return Model(inputs=[inputs], outputs=[outputs], name='deeplabv3plus_fallback')
    
    def _build_segformer_b0_architecture(self) -> Model:
        """Architecture Segformer-B0 simplifiée - EXACTE du notebook d'entraînement"""
        inputs = layers.Input(shape=INPUT_SHAPE)
        
        # Patch embedding
        x = layers.Conv2D(32, 7, strides=4, padding='same')(inputs)  # 128x256
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        
        # Stage 1
        x = layers.Conv2D(64, 3, strides=2, padding='same')(x)  # 64x128
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        
        # Stage 2
        x = layers.Conv2D(160, 3, strides=2, padding='same')(x)  # 32x64
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        
        # Stage 3
        x = layers.Conv2D(256, 3, strides=2, padding='same')(x)  # 16x32
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        
        # Decoder simple (simplified version)
        x = layers.UpSampling2D((2, 2))(x)  # 32x64
        x = layers.Conv2D(160, 3, padding='same', activation='relu')(x)
        
        x = layers.UpSampling2D((2, 2))(x)  # 64x128
        x = layers.Conv2D(64, 3, padding='same', activation='relu')(x)
        
        x = layers.UpSampling2D((2, 2))(x)  # 128x256
        x = layers.Conv2D(32, 3, padding='same', activation='relu')(x)
        
        x = layers.UpSampling2D((4, 4))(x)  # 512x1024
        x = layers.Conv2D(16, 3, padding='same', activation='relu')(x)
        
        # Output (8 classes Cityscapes)
        outputs = layers.Conv2D(NUM_CLASSES, 1, activation='softmax', name='output_segmentation')(x)
        
        return Model(inputs=[inputs], outputs=[outputs], name='segformer_b0_fallback')
    
    def _get_model_architecture_description(self, model_name: str) -> str:
